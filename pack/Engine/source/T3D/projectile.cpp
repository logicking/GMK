...
#include "T3D/vehicles/physicsVehicle.h"
...
IMPLEMENT_CALLBACK( ProjectileData, update_check, void, ( Projectile* obj), ( obj ),
   "Called every tick if projectile is guided." );
...
ProjectileData::ProjectileData()
{
   ...
   vehicleExplosion = NULL;
   vehicleExplosionId = 0;
...
void ProjectileData::initPersistFields()
{
   ...
   addField("bloodExplosion", TYPEID< ExplosionData >(), Offset(bloodExplosion, ProjectileData));
   ...
   addField("vehicleExplosion", TYPEID< ExplosionData >(), Offset(vehicleExplosion, ProjectileData));
   ...
   addField("explodeOnExplodeTime", TypeBool, Offset(explodeOnExplodeTime, ProjectileData));
   ...
   addField("isfluid", TypeBool, Offset(isfluid, ProjectileData));
...
*/
   ...
   addNamedFieldV(explosionTolerance, TypeF32, ProjectileData, new FRangeValidator(0, 100));
   addNamedFieldV(trackDelay, TypeS32, ProjectileData, new FRangeValidator(0, 100000));
   addField("trackingAngle", TypeF32, Offset(trackingAngle, ProjectileData));
...
bool ProjectileData::preload(bool server, String &errorStr)
{
   ...
   if (!vehicleExplosion && vehicleExplosionId != 0)
	    if (Sim::findObject(vehicleExplosionId, vehicleExplosion) == false)
         Con::errorf(ConsoleLogEntry::General, "ProjectileData::onAdd: Invalid packet, bad datablockId(vehicleExplosion): %d", vehicleExplosionId);
...
void ProjectileData::packData(BitStream* stream)
{
   ...
   stream->write(mPierceWidth);
   stream->write(mPlayerRearScanDist);
   ...
   if (stream->writeFlag(vehicleExplosion != NULL))
	    stream->writeRangedU32(vehicleExplosion->getId(), DataBlockObjectIdFirst,
	                                                      DataBlockObjectIdLast);
...
//    stream->writeRangedU32(fadeDelay, 0, Projectile::MaxLivingTicks);
   ...
   if(stream->writeFlag(isGuided))
   {
	   ...
	   stream->write(explosionTolerance);
	   stream->write(trackDelay);
	   stream->write(trackingAngle);
   }
...
void ProjectileData::unpackData(BitStream* stream)
{
   ...
   stream->read(&mPierceWidth);
   stream->read(&mPlayerRearScanDist);
   ...
   if (stream->readFlag())
      vehicleExplosionId = stream->readRangedU32(DataBlockObjectIdFirst,  DataBlockObjectIdLast); 
...
//    fadeDelay = stream->readRangedU32(0, Projectile::MaxLivingTicks);
   ...
   if(isGuided)
   {
	   ...
	   stream->read(&explosionTolerance);
	   stream->read(&trackDelay);
	   stream->read(&trackingAngle);
   }
   ...
   if (!fromWater && toWater)     // entering water
   {
      ...
      if (gClientContainer.castRay(from, to, WaterObjectType, &rInfo))
      {
		  if(isClientObject())
		  {
			  ...
			  onCollision(rInfo.point, rInfo.normal, NULL, rInfo.HitBoxNum);
      }
   }
}
...
void Projectile::explode(const Point3F& p, const Point3F& n, const Point3F &impactNormal, const U32 collideType, U32 pMatType, bool transDec, SceneObject* transObj )
{
   ...
   else 
   {
      ...
      if (mDataBlock->vehicleExplosion && (collideType & VehicleObjectType))
      {
         pExplosion = new Explosion;
         pExplosion->onNewDataBlock(mDataBlock->vehicleExplosion, false);
      }
      else
	  ...
	  else if(collideType & (PlayerObjectType))
	  {
		  ...
		  {
			  pExplosion = new Explosion;
			  pExplosion->onNewDataBlock(mDataBlock->bloodExplosion, false);
...
{         
   ...
   {
     onCollision(newPosition, Point3F(0.0, 0.0, 1.0), this, rInfo.HitBoxNum);
   ...
   if(mDataBlock->explodeOnWater && !mDataBlock->stoppedByWater)
	...
	{
	  if (isServerObject() && (bool)mTarget && !explodeAnyway)
	   {
		 Point3F targetPos = mTarget->getPosition(); 
		 Point3F targetVec = targetPos - mCurrPosition;
		 F32 distance = targetVec.len();

		 if (distance < mDataBlock->explosionTolerance)
		  {
			explodeAnyway = true;
		  }
	}
		...
		PhysicsVehicle* targetVehicle2 = static_cast<PhysicsVehicle*>(mTarget);
		if(targetVehicle2 && !mDataBlock->isHeatSeeker)
		{
			if(targetVehicle2->getJammerPresent() && targetVehicle2->getJammerActive())
				mTarget = NULL;
		}
   ...
   if( mDataBlock->isBallistic )
   {
	   ...
	   F32 dV = (1-mDataBlock->ballisticCoefficient);
	   Point3F d(mCurrVelocity.x * dV,mCurrVelocity.y * dV, 9.81f * mDataBlock->gravityMod + mCurrVelocity.z * dV);
	   mCurrVelocity -= d;
   }
	...
	{
//		Sky* object;
//		Sky* pSky = static_cast<Sky*>(object);
//		if (pSky)
//		{
//			Point3F windvel = pSky->getWindVelocity();
//			mCurrVelocity += windvel * mDataBlock->windEffect;
//		}
	}
   ...
   if ( hit )
   {
      ...
      if ( mCurrTick > mDataBlock->armingDelay || mDataBlock->armingDelay == 0 )
      {
		 ...
		 if( rInfo.object->getTypeMask() & (InteriorObjectType | StaticShapeObjectType | StaticObjectType))
		 {
			 if(armorPiercing)
			 {
		 ...
		 else if( rInfo.object->getTypeMask() & PlayerObjectType )
		 {
			 ...
			 if (getContainer()->castRay(mBloodRearPosition, mBloodObjPosition, csmStaticCollisionMask, &playerRearInfo) == true)
			 {
				 ...
				 if(goreMode)
					 addPierceDecal(playerRearInfo.point, playerRearInfo.normal, secImpactNormal, PlayerObjectType, 0);
			 }
		 }
		 onCollision(mPosition, mNormal, rInfo.object, rInfo.HitBoxNum);
		 explode(mPosition, mNormal, impactNormal, objectType, 0, 0, rInfo.object);
      }
      else
      {
         if ( mDataBlock->isBallistic )
     ...
     if(mDataBlock->isfluid)
	 {
	    MatrixF xform(true);
            ...
            mCurrPosition    = rInfo.point;
               ...
               if(rInfo.object->getTypeMask() & csmStaticCollisionMask){mCurrVelocity  = Point3F(0,0,0);}
	           U32 objectType = rInfo.object->getTypeMask();
   ...
   if(mDataBlock->isGuided)
	   //mDataBlock->update_check_callback( this );
       Con::executef(mDataBlock, "updateCheck", getIdString());
}
...
void Projectile::unpackUpdate(NetConnection* con, BitStream* stream)
{
   ...
   if(addPierce == true)
   {
	   addPierceDecal(mPiercePoint, mPierceNormal, mPierceINormal, mCollideHitType, 0);
   }
   ...
   if ( stream->readFlag() ) // ExplosionMask
   {
      ...
      explode(explodePoint, explodeNormal, mImpactNormal, mCollideHitType, 0, 0, 0); //RJN
   }
...
ConsoleMethod( Projectile, setNewTarget, void, 3, 3, "obj.setNewTarget(GameBase obj);" 
	"The a new target for this projectile. Also possible after object creation!")
{
	ShapeBase* tptr;
	if(Sim::findObject(argv[2], tptr))
	{
		object->mTarget = tptr;
	}
}	

ConsoleMethod( Projectile, setEvaded, void, 2, 2, "obj.setEvaded();" 
	"Set the target to nothing. Used for when flares distract a missile.")
{
		object->mTarget = 0;
}

ConsoleMethod( Projectile, setExplodeNow, void, 2, 2, "obj.setExplodeNow();" 
				"Tells the projectile to explode immediately.")
{
	object->expire();
}

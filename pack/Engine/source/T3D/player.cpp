...
void Player::processTick(const Move* move)
{
   ...
   PhysicsWorld *world = PHYSICSMGR->getWorld( isServerObject() ? "server" : "client" );
   bool isPhysics = world;

   gServerContainer.initRadiusSearch( getPosition(), 1.0f, VehicleObjectType );
   ...
   while (objectId != 0)
	 {
      if ( mPhysicsRep )
	   {
		 //Con::printf("===== mPhysicsPlayer DISABLED =====");
		 SAFE_DELETE( mPhysicsRep );
	   }
   ...
   MatrixF mat;
   this->getEyeTransform( &mat );
   VectorF eyeVector;
   mat.getColumn( 1, &eyeVector );
   VectorF eyePos;
   mat.getColumn( 3, &eyePos );
   eyeVector *= 2.5f;
   Point3F end = eyePos + eyeVector;
   RayInfo ri; 

   bool hit;
   hit = gClientContainer.castRay(getPosition(), end, VehicleObjectType, &ri);
    ...
    if (hit || isMounted())
	 ...
	 {
      if ( !mPhysicsRep && isPhysics )
	   {
		 //Con::printf("===== mPhysicsPlayer ENABLED 2 =====");
         mPhysicsRep = PHYSICSMGR->createPlayer();
         mPhysicsRep->init(mDataBlock->physicsPlayerType,
                              mDataBlock->boxSize,
                              mDataBlock->runSurfaceCos,

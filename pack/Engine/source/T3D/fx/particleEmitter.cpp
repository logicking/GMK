...
#include "T3D/groundPlane.h"//DS ADDED KILLING PARTICLES
#include "environment/waterObject.h"//DS ADDED KILLING PARTICLES
...
void ParticleEmitterData::initPersistFields()
{
   addGroup( "ParticleEmitterData" );
      ...
      addField("enableHeat",           TypeBool,   Offset(enableHeat,         ParticleEmitterData));
      ...
      addField("heatName",           TypeFilename, Offset(heatName,           ParticleEmitterData));//LIMAN
...
void ParticleEmitterData::packData(BitStream* stream)
{
   ...
   if (stream->writeFlag(heatName != 0))
     stream->writeString(heatName);
...
void ParticleEmitterData::unpackData(BitStream* stream)
{
   ...
   heatName = (stream->readFlag()) ? stream->readSTString() : 0;
...
bool ParticleEmitterData::preload(bool server, String &errorStr)
{
   ...
   if (!server)
   {
     ...
     if (enableHeat && heatName && heatName[0])
     {
       heatHandle = GFXTexHandle(heatName, &GFXDefaultStaticDiffuseProfile, avar("%s() - heatHandle (line %d)", __FUNCTION__, __LINE__));
       if (!heatHandle)
       {
         errorStr = String::ToString("Missing particle emitter heat texture: %s", heatName);
         return false;
       }
     }
...
ParticleEmitter::ParticleEmitter()
{
   ...
   mControlClient = 0;//DS ADDED KILLING PARTICLES
...
void ParticleEmitter::prepRenderImage(SceneRenderState* state)
{
   ...
   if (mDataBlock->heatHandle)  
   {   
	 ri->heatTex = &*(mDataBlock->heatHandle);
     ri->isHeatpass = true;
   }
...
void ParticleEmitter::update( U32 ms )
{
   ...
   //DS ADDED KILLING PARTICLES
   if(part->dataBlock->detectCollisions == true)
   {
   //do our stuff with mCurrVelocity
   Point3F mCurrVelocity = part->vel;
   //No matter what we want to KILL particles  that are under the terrain for the sake of speed.


   Point2F Theight;
   Theight.x=part->pos.x;
   Theight.y=part->pos.y;
   F32 height = getTerrainHeight(Theight);

   if (height > (part->pos.z))
   {
	   part->currentAge = part->totalLifetime;
	   return;// If we killed it, skip on out of here...
   }
   ...
   if(part->dataBlock->detectCollisions == true)
   {
	   ...
	   // collision test... 
	   if(gServerContainer.castRay(part->pos, part->pos + ((part->vel * t) * part->dataBlock->colDetectDistAdj), 
		   TerrainObjectType | StaticObjectType | ItemObjectType
		   |InteriorObjectType | ShapeBaseObjectType | ProjectileObjectType, &rInfo))
	   {
		   ////First, reflect our velocity around the normal...
		   Point3F bounceVel = mCurrVelocity - rInfo.normal * 
			   (mDot( mCurrVelocity, rInfo.normal ) * 2.00);
		   mCurrVelocity = bounceVel;
		   
		   ////Add in surface friction...
		   Point3F tangent = bounceVel - rInfo.normal * mDot(bounceVel, rInfo.normal);
		   mCurrVelocity  -= tangent * part->dataBlock->friction;
		   
		   ////Now, take elasticity into account 
		   mCurrVelocity *= part->dataBlock->elasticity;
		   
		   ////Give it a slight random bounce 
		   mCurrVelocity = MathUtils::randomDir(  
			   mCurrVelocity ,-30  ,30  ,-30  ,30  );
		   //------------------------------------------------------------------------------------script call back
		   if(rInfo.object->getTypeMask() & WaterObjectType) //Only want datablock Shape objects to callback
		   {
			   onCollision(rInfo.point, rInfo.normal, rInfo.object, mControlClient);
		   }

		   if(rInfo.object->getTypeMask() & TerrainObjectType) //Only want datablock Shape objects to callback
		   {
			   onCollision(rInfo.point, rInfo.normal, rInfo.object, mControlClient);
		   }

		   if(rInfo.object->getTypeMask() & ShapeBaseObjectType) //Only want datablock Shape objects to callback
		   {
			   onCollision(rInfo.point, rInfo.normal, rInfo.object, mControlClient);
		   }
		   //------------------------------------------------------------------------------------script call back
		   
		   if(part->dataBlock->killedByCollision == true)
		   {
			   part->currentAge = part->totalLifetime;
			   return;
		   }
   ...
   //Put our mCurrVelocity back into particle->vel and we will be done...
   part->vel = mCurrVelocity;
   }
   //DS ADDED KILLING PARTICLES <<
...
//DS ADDED KILLING PARTICLES
void ParticleEmitter::onCollision(const Point3F& hitPosition, const Point3F& hitNormal, SceneObject* hitObject, S32 client)
{
      char *posArg = Con::getArgBuffer(64);
      char *normalArg = Con::getArgBuffer(64);
	  char *clientArg = Con::getArgBuffer(10);
	  ...
	  dSprintf(clientArg, 10, "%d", client);

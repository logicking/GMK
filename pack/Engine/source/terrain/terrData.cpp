...
bool TerrainBlock::onAdd()
{
   ...
   if (isClientObject())
   {
      ...
      if ( Platform::compareModifiedTimes( baseCachePath, mTerrFileName ) < 0 )
         _updateBaseTexture( false );//DS>> WAS TRUE, FALSE STOPS THE OVERWRITING
   ...
   if ( PHYSICSMGR )
   {
      ...
      Con::errorf(ConsoleLogEntry::General, "=====CREATING STATIC TERRAIN=====");
   }
    
   //.logicking >>
   else
   {
      Con::errorf(ConsoleLogEntry::General, "=====BUILDING PHYS TERRAIN=====");
	   ...
	   if(physics)
	   {
		   ...
		   SimObject* missionAreaObj = 	Sim::findObject("theMissionArea");
		   MissionArea * obj = dynamic_cast<MissionArea*>(missionAreaObj); 
		   if (obj)
		   {
			   AssertFatal(obj, "");
			   F32 minH,maxH;
			   getMinMaxHeight(&minH,&maxH);
			   ...
			   Box3F box = Box3F(area.point.x, area.point.y, -10.f, 
				   ...
				   /*fixedToFloat(gridMap[BlockShift]->maxHeight)*/maxH);

			   PolyListContext context;

			   bool res =  buildPolyList(context, mPhysPolyList, box, SphereF());
...
//<< Jonajoint Added
 // tb = the terrain block to match to, edge is the edge number, 0 being north, 1 east, 2 south, 3 west
void TerrainBlock::matchEdge(TerrainBlock* terrain, U32 edge,bool slope, U32 smoothBorder,F32 weight)
{
	U32 edge2 = edge+2;
	if(edge2 > 3)
		edge2 -= 4;
	...
	if(edge==0)
	{
		...
		edgeXMax=blockSize;
		edge2Y=blockSize-1;
		edge2X=0;// takes value from edgeX for this orientation so just set to 0
	}else if(edge==1)
	{
		edgeYMin=0;
		edgeYMax=blockSize;
		...
		edgeXMax=blockSize;
		edge2Y=0;// takes value from edgeY for this orientation so just set to 0
		edge2X=0;
	}else if(edge==2)
	{
...
//					Con::printf("Doing East Smooth");
	...
	// smoothing ( Kind of :p )
	// TODO This currently uses all the points on the edge to work out the smoothing in one pass.
	// (Change so it moves along the edge using only a small portion at a time)

	// This Linear Regression is adapted from Dan Kellers 'Better smoothing for terrain editor' resource
	// http://www.garagegames.com/community/resources/view/21479

	// fill a buffer with values
	if(smoothBorder > 0 && weight >0){
		Vector<SmoothBufferInfo> sBuff;
		if(edge==0){
			for ( U32 y = edgeYMin; y < edgeYMax+smoothBorder; y++ )
			{
				for ( U32 x = edgeXMin; x < edgeXMax; x++ )
				{
					SmoothBufferInfo sbi;
					sbi.height = fixedToFloat( mFile->getHeight(x,y) );
					sbi.x = x;
					sbi.y = y;
					sBuff.push_back(sbi);
				}
			...
			for ( U32 y = edgeYMin; y < edgeYMax; y++ )
			{
				for ( U32 x = edgeXMin-smoothBorder; x < edgeXMax; x++ )
				{
					SmoothBufferInfo sbi;
					sbi.height = fixedToFloat( mFile->getHeight(x,y) );
					sbi.x = x;
			...
			for ( U32 y = (edgeYMax-1)-smoothBorder; y < edgeYMax; y++ )
			{
				for ( U32 x = edgeXMin; x < edgeXMax; x++ )
				{
					SmoothBufferInfo sbi;
			...
			for ( U32 y = edgeYMin; y < edgeYMax; y++ )
			{
				for ( U32 x = edgeXMin; x < edgeXMax+smoothBorder; x++ )
				{
					SmoothBufferInfo sbi;
					sbi.height = fixedToFloat( mFile->getHeight(x,y) );
					sbi.x = x;
	...
	//perform simple 2d lin reg on x&z and y&z:  
      ...
      F32 z;  
      Point2F pos;  
      for(U32 k = 0; k < sBuff.size(); k++)  
      {  
         pos = Point2F(sBuff[k].x, sBuff[k].y);  
         z = sBuff[k].height;  
      ...
      avgSqr /= sBuff.size();  
      avgPos /= sBuff.size();  
      avgHeight /= sBuff.size();  
  
      Point2F avgSlope = (prod - avgPos*avgHeight)/(avgSqr - avgPos*avgPos);  
  
      F32 goalHeight;  
      for(U32 i = 0; i < sBuff.size(); i++)  
      {  
		  // ignore the very edge points
		  if(sBuff[i].x != 0 && sBuff[i].x != (blockSize-1) && sBuff[i].y != 0 && sBuff[i].y != (blockSize-1)){
				goalHeight = avgHeight + (sBuff[i].x - avgPos.x)*avgSlope.x +  
					(sBuff[i].y - avgPos.y)*avgSlope.y;  
				sBuff[i].height += (goalHeight - sBuff[i].height) * weight;
				mFile->setHeight( sBuff[i].x, sBuff[i].y, floatToFixed(sBuff[i].height) );
		  }
      }  
  	}
...
DefineEngineMethod( TerrainBlock, matchEdge, void, ( TerrainBlock* terrBlock, U32 edge, bool slope, U32 smoothBorder,F32 weight),,
				   "@brief Matches an edge on this terrain block with the opposite edge on the one supplied.\n\n"
				   "@param terrBlock = the TerrainBlock to match to.\n\n"
				   "@param edge = The edge on this terr.\n\n"
				   "@param slope = do linear slope smoothing.\n\n"
				   "@param smoothBorder = amount of edge to do smoothing operations on.\n\n"
				   "@param weight = amount to do linear regression (WIP currently uses all edge points.)")
{
	object->matchEdge(terrBlock,edge,slope,smoothBorder,weight);
}
//>> Jonajoint End

//DS ADDED KILLING PARTICLES
